...
public class NearestTreeSensor<E extends SpiritEntity> extends ExtendedSensor<E> {
    ...
    public static boolean isTreeSoil(Level level, BlockPos pos) {
        return level.getBlockState(pos).is(Blocks.TREE_SOIL);
    }
    ...
    protected void doTick(@NotNull ServerLevel level, @NotNull E entity) {
        ...
        if (!potentialStumps.isEmpty()) {
            ...
            for (var potentialStump : potentialStumps) {
                ...
                if (Occultism.DEBUG.debugAI) {
                    Networking.sendToTracking(entity, new MessageSelectBlock(potentialStump, 5000, Color.WHITE));
                }
    ...
    private boolean isTree(Level level, BlockPos potentialStump) {
        if (isLog(level, potentialStump)) {
            ...
            while (!level.isEmptyBlock(topOfTree.above()) && topOfTree.getY() < level.getMaxY()) {
                ...
                if (level.isEmptyBlock(topOfTree.above()) && topOfTree.getY() < level.getMaxY()) {
                    for (Direction direction : Plane.HORIZONTAL) {
                        if (!level.isEmptyBlock(topOfTree.above().relative(direction))) {
                            topOfTree = topOfTree.relative(direction);
                            break;
                        }
                    }
                }
